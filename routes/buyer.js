...
router.get("/login", async (req, res) => {
    if (req.cookies.user) {
      ...
      if (findId) {
        ...
        res.render("buyer/login", { wrong: "", emailExist: true });
      }
    } else {
      res.render("buyer/login", { wrong: "", emailExist: true });
    }
  ...
  router.post("/login", async (req, res) => {
    ...
    if (checkData) {
      if (password == checkData.dataValues.password) {
        ...
        res.render("buyer/login", { wrong: "Wrong password", emailExist: true });
      }
    } else {
      res.render("buyer/login", { wrong: "", emailExist: false });
    }
  });
  ...
  router.get("/signup", async (req, res) => {
    if (req.cookies.buyer) {
      ...
      if (findId) {
        ...
        res.clearCookie("buyer");
        res.render("buyer/signup", { exist: false, wrongPassword: false });
      }
    } else {
      res.render("buyer/signup", { exist: false, wrongPassword: false });
    }
  ...
  router.post("/signup", async (req, res) => {
    ...
    if (password !== confirm) {
      res.render("buyer/signup", { exist: false, wrongPassword: true });
    } else {
      if (result) {
        res.render("buyer/signup", { exist: true, wrongPassword: false });
      } else {
